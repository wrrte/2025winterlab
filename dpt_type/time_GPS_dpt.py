...
def GPS_dpt(model_path, cap, current_gps, heading, ref_distance, FOV):
    ...
    input_file = "/mnt/hdd_4A/choemj/2025winterlab/roadview/골목_2.png"
    image = util.io.read_image(input_file)
    ...
    return (start0-start, start1-start0, start2-start1, start3-start2)
...
if __name__ == "__main__":
    ...
    GPS_dpt(model_path, cap, current_gps, heading, reference_distance, FOV)
    
    for i in range(N):
        

        x0,x1,x2,x3=GPS_dpt(model_path, cap, current_gps, heading, reference_distance, FOV)
        ll[0]+=x0
        ll[1]+=x1
        ll[2]+=x2
        ll[3]+=x3

    print(ll[0]/N, ll[1]/N, ll[2]/N, ll[3]/N)
